...
function NavBar({ view, infoDataGold, infoDataUsername, partyData }) {
    ...
    return(
        <nav className='navbar relative h-full w-full bg-base-300 items-center justify-center gap-[10vw] border-b pl-[1vw] pr-[1vw]'>
            ...
            <div className='flex flex-grow relative h-fit w-full items-center justify-center badge badge-primary p-2 text-xl'>
                {view === 'town' && `In Town`}
                {view === 'dungeon' && `In Dungeon`}
                {view === 'battle' && `In Battle`}
            </div>
            <div className='flex flex-end relative h-fit w-full items-center justify-center p-1 gap-[2vw]'>
                <div className='badge badge-warning p-5 relative w-fit'>
                    <Coins size={20}/>
                    <text className='w-full text-xl justify-items-center'>{infoDataGold}</text>
                </div>
